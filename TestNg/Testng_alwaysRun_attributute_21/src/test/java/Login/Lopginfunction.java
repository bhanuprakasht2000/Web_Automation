...
public class Lopginfunction {
	...
	//Priority will be overriden by the dependsOnMehthods
	//Not supposed to delete the main test or use the @ignore tag on it
	//@alwayRun==>Used for the running the other dependents event the dependent method fails or pass also
	//login fails but next executes
	@Test(priority = 3)
	...
	@Test(priority=1,dependsOnMethods = {"loginwithvalidceredentials"},alwaysRun=true)
	...
	@Test(priority=1,dependsOnMethods = {"loginwithvalidceredentials","searchedwishlist"})
